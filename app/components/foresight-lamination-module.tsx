...
const DEPT_KEY: DeptKey = 'lamination';

export default function ForesightLaminationModule() {
  ...
  const MiniPartsList = ({ boatName, dept }: { boatName: string; dept: DeptKey }) => {
    ...
    return (
      <div className={`mt-2 ml-4 border-l-2 border-orange-200 pl-3 space-y-1`}>
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-800">Lamination Schedule</h2>
        <div className="flex items-center space-x-4">
          <button className="text-sm font-medium text-blue-600 hover:text-blue-800">+ Add Parts</button>
          <label className="flex items-center text-sm text-gray-600 cursor-pointer">
            <input type="checkbox" checked={isDense} onChange={() => setIsDense(!isDense)} className="mr-2 h-4 w-4 rounded text-orange-600 focus:ring-orange-500" />
            Dense
          </label>
        </div>
      </div>
              ...
              {tasksByWeek[week].map((task, index) => (
                ...
                >
                  <div
                    ...
                    className={`p-3 rounded-lg shadow-sm cursor-grab ${isDense ? 'py-2' : ''} ${selectedIds.has(task.id) ? 'ring-2 ring-orange-500 bg-orange-100' : 'bg-white'} ${draggingId === task.id ? 'opacity-50' : ''}`}>
